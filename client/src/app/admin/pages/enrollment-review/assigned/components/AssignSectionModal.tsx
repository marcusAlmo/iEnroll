...
import { faUserPlus, faChevronDown, faExclamationTriangle } from '@fortawesome/free-solid-svg-icons';
...
/**
 * AssignSectionModal Component
 * 
 * A modal dialog that allows administrators to assign a student from the "Unassigned" section
 * to a proper section. It provides a searchable dropdown of available sections from the selected grade level.
 * 
 * Features:
 * - Displays student information
 * - Provides a searchable combobox for section selection
 * - Only allows assignment for students with "Accepted" application status
...
export default function AssignSectionModal() {
  ...
  const canAssign = selectedStudent?.applicationStatus === "Accepted";
  
  // Filter out the "Unassigned" section from the available sections
  const availableSections = sections.filter(section => section.sectionName !== "Unassigned");
  ...
  const handleConfirm = () => {
    if (selectedNewSection && selectedStudent) {
      console.log(`Assigning student ${selectedStudent.studentName} to section ${selectedNewSection.sectionName}`);
      closeModal();
    }
  }
              ...
              <Dialog.Panel className="w-full max-w-md p-10 overflow-visible text-left align-middle transition-all transform bg-white shadow-xl rounded-2xl">
                ...
                >
                  <div className='flex flex-col justify-center text-center'>
                    <FontAwesomeIcon icon={faUserPlus} className="text-secondary text-8xl" />
                    <h2 className="text-2xl font-semibold text-primary">
                      Assign Student to Section
                    </h2>
                    <p className="mx-5 text-center text-sm font-normal text-text-2">
                      This action will assign the student to a proper section.
                    </p>
                  </div>
                </Dialog.Title>
                ...
                {/* Modal content */}
                <div className="mt-10">
                  {/* Warning message if student cannot be assigned */}
                  {!canAssign && (
                    <div className="mb-6 p-4 bg-warning/20 rounded-lg border border-warning">
                      <div className="flex items-center">
                        ...
                        <p className="text-sm text-text">
                          Only students with <span className="font-semibold">Accepted</span> application status can be assigned to a section.
                        </p>
                      </div>
                    </div>
                  )}
  
                  {/* Section selector - only shown if student can be assigned */}
                  {canAssign && (
                    <div className="mt-6">
                      <label htmlFor="section-combobox" className="block text-sm font-medium text-text mb-2">
                        Select Section for <span className='font-semibold'>{selectedStudent?.studentName}</span>
                      </label>
                      <Combobox 
